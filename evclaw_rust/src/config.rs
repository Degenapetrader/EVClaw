...
impl Config {
    pub fn load(overrides: RuntimeOverrides) -> Result<Self> {
        ...
        Ok(Self {
            ...
            min_hold_hours: env_f64("EVCLAW_MIN_HOLD_HOURS", 1.0).max(0.0),
            sl_atr_multiplier: env_f64("EVCLAW_SL_ATR_MULTIPLIER", 2.0).max(0.1),
            tp_atr_multiplier: env_f64("EVCLAW_TP_ATR_MULTIPLIER", 3.0).max(0.1),
            base_notional_usd,
            ...
            state_dir: PathBuf::from(env_string(
                "EVCLAW_STATE_DIR",
                "/root/clawd/skills/EVClaw/evclaw_rust/state-live",
            )),
            ...
            scored_wallets_path: PathBuf::from(env_string(
                ...
                "/root/clawd/skills/EVClaw/evclaw_rust/scored_wallets.json",
            )),
...
fn resolve_env_path() -> Result<PathBuf> {
    ...
    let candidates = [
        cwd.join(".env"),
        cwd.join("../.env"),
        PathBuf::from("/root/clawd/skills/EVClaw/evclaw_rust/.env"),
    ];
    candidates
        .into_iter()
