...
struct ProxyHttpClient {
    direct: Client,
    proxies: Arc<Vec<ProxyClient>>,
    next_proxy_idx: Arc<AtomicUsize>,
}
...
const ORDER_FILL_SETTLE_RETRIES: usize = 8;
const ORDER_FILL_SETTLE_DELAY_MS: u64 = 500;
...
impl ProxyHttpClient {
    fn new(timeout_secs: u64, proxies: &[String]) -> Result<Self> {
        ...
        for proxy_url in proxies {
            ...
            let client = match Client::builder()
                .timeout(Duration::from_secs(timeout_secs))
                .proxy(Proxy::all(trimmed).with_context(|| {
                    format!("invalid proxy configured: {}", safe_proxy_label(trimmed))
                })?)
                .build()
            {
                Ok(client) => client,
                Err(err) => {
                    warn!(
                        "[HL] skipping proxy {} because client build failed: {}",
                        safe_proxy_label(trimmed),
                        err
                    );
                    continue;
                }
            ...
            proxy_clients.push(ProxyClient {
                label: safe_proxy_label(trimmed),
                client,
            });
        }
        ...
        Ok(Self {
            direct,
            proxies: Arc::new(proxy_clients),
            next_proxy_idx: Arc::new(AtomicUsize::new(0)),
        })
    }

    async fn post_json(&self, url: &str, payload: &Value) -> Result<Value> {
        let direct_err = match post_json_with_client(&self.direct, url, payload).await {
            Ok(value) => return Ok(value),
            Err(err) => err,
        };

        if !should_try_proxy_fallback(&direct_err) || self.proxies.is_empty() {
            return Err(direct_err);
        }

        debug!(
            "[HL] public request hit fallback path for {}: {}",
            url, direct_err
        );
        let start_idx = self.next_proxy_idx.fetch_add(1, Ordering::Relaxed) % self.proxies.len();
        let mut last_err = direct_err;

        for offset in 0..self.proxies.len() {
            let idx = (start_idx + offset) % self.proxies.len();
            let proxy = &self.proxies[idx];
            match post_json_with_client(&proxy.client, url, payload).await {
                Ok(value) => return Ok(value),
                Err(err) => {
                    debug!(
                        "[HL] proxy fallback failed for {} via {}: {}",
                        url, proxy.label, err
                    );
                    last_err = err;
                }
...
impl InfoClient {
    pub fn new(cfg: &Config) -> Result<Self> {
        let public_http = ProxyHttpClient::new(cfg.request_timeout_secs, &cfg.hyperliquid_proxies)?;
        let http = public_http.direct.clone();
        Ok(Self {
    ...
    pub async fn fetch_live_account_positions(&self) -> Result<HashMap<String, LivePosition>> {
        ...
        let data = self
            .public_http
            .post_json(&self.public_info_url, &payload)
            .await?;
    ...
    pub async fn fetch_live_account_summary(&self) -> Result<Option<AccountSummary>> {
        ...
        let data = self
            ...
            .post_json(&self.public_info_url, &payload)
            .await?;
        let mut summary = parse_account_summary(user.clone(), &data);
        let spot_payload = json!({ "type": "spotClearinghouseState", "user": user });
        if let Ok(spot_data) = self
            .public_http
            .post_json(&self.public_info_url, &spot_payload)
            .await
        {
            if let Some(spot_summary) = parse_spot_usdc_summary(user.clone(), &spot_data) {
                if spot_summary.available_margin > summary.available_margin {
                    summary = spot_summary;
                } else if summary.account_value <= 0.0
                    && summary.margin_used <= 0.0
                    && summary.available_margin <= 0.0
                {
                    summary = spot_summary;
                }
            }
...
pub struct ExecutionClient {
    ...
    symbol_max_leverage: HashMap<String, f64>,
...
impl ExecutionClient {
    pub async fn new(cfg: &Config) -> Result<Self> {
        ...
        let exchange = if cfg.dry_run || cfg.acp_mode {
            None
        } else {
            let private_key = cfg
        ...
        Ok(Self {
            ...
            symbol_max_leverage: HashMap::new(),
    ...
    pub fn register_market_meta(&mut self, meta: &HashMap<String, MarketMeta>) {
        for (symbol, market) in meta {
            ...
            self.symbol_max_leverage
                .insert(symbol.clone(), market.max_leverage);
    ...
    pub fn max_leverage(&self, symbol: &str) -> f64 {
        self.symbol_max_leverage.get(symbol).copied().unwrap_or(1.0)
    }

    pub fn exchange_symbol_for(&self, symbol: &str) -> String {
        self.exchange_symbol(symbol).unwrap_or(symbol).to_string()
    }

    pub fn set_trading_address(&mut self, address: Option<String>) {
        self.trading_address = address;
    }
    ...
    pub async fn place_stop_order(
        ...
        let (is_market_trigger, tpsl_kind) = trigger_order_mode(order_type);
        let req = ClientOrderRequest {
            ...
            order_type: ClientOrder::Trigger(ClientTrigger {
                // Stop-loss stays market-style. Take-profit stays trigger-limit.
                is_market: is_market_trigger,
                trigger_px: normalized_trigger_px,
                tpsl: tpsl_kind.to_string(),
            }),
        };
    ...
    pub async fn place_market_order(
        ...
        while start.elapsed() < Duration::from_secs(self.chase_limit_timeout_secs) {
            ...
            if current_order_id.is_none() {
                ...
                if !response.success {
                    return self
                        .failed_entry_result_with_cleanup(
                            symbol,
                            response
                                .order_id
                                .as_deref()
            ...
            if let (Some(order_id), Some(active_limit_px)) = (current_order_id, current_limit_px) {
                ...
                if ticks_moved >= self.chase_replace_ticks {
                    let cancel_outcome = self
                        .cancel_order_with_confirmation_details(symbol, &order_id.to_string())
                        .await?;
                    if matches!(cancel_outcome, CancelConfirmOutcome::Failed) {
                        return self
                            .failed_entry_result_with_cleanup(
                                symbol,
                                Some(order_id),
                    ...
                    if matches!(cancel_outcome, CancelConfirmOutcome::AlreadyGone) {
                        return self
                            .failed_entry_result_with_cleanup(
                                symbol,
                                Some(order_id),
    ...
    async fn cancel_order_with_confirmation(
        ...
        Ok(!matches!(
            self.cancel_order_with_confirmation_details(symbol, order_id)
                .await?,
            CancelConfirmOutcome::Failed
        ))
    }

    async fn cancel_order_with_confirmation_details(
        &mut self,
        symbol: &str,
        order_id: &str,
    ) -> Result<CancelConfirmOutcome> {
        ...
        if !self.is_order_still_open(symbol, order_id).await? {
            return Ok(if cancelled {
                CancelConfirmOutcome::ConfirmedCanceled
            } else {
                CancelConfirmOutcome::AlreadyGone
            });
        }
        ...
        Ok(CancelConfirmOutcome::Failed)
    }
    ...
    async fn failed_entry_result_with_cleanup(
        &mut self,
        symbol: &str,
        order_id: Option<u64>,
        filled_total: f64,
        ...
        Ok(OrderResult {
            success: false,
            order_id: order_id.map(|oid| oid.to_string()),
            filled_size: filled_total,
            filled_price: if filled_total > 0.0 {
                filled_notional / filled_total
            } else {
    ...
    async fn refresh_price_cache(&mut self) -> Result<()> {
        match self
            .public_http
            .post_json(&self.public_info_url, &json!({ "type": "allMids" }))
            .await
        {
            Ok(data) => {
...
async fn fetch_wallet_state(
    ...
    let data = post_json_with_client(&client, &url, &payload).await?;
...
fn parse_spot_usdc_summary(wallet: String, data: &Value) -> Option<AccountSummary> {
    let balances = data.get("balances").and_then(Value::as_array)?;
    let usdc_balance = balances.iter().find(|balance| {
        balance
            .get("coin")
            .and_then(Value::as_str)
    ...
    let total = usdc_balance
        .get("total")
        .and_then(value_as_f64)
        .unwrap_or(0.0);
    let hold = usdc_balance
        .get("hold")
        .and_then(value_as_f64)
        .unwrap_or(0.0);
    let token_id = usdc_balance
        .get("token")
        .and_then(Value::as_u64)
        .unwrap_or(0);

    let available_after_maintenance = data
        ...
        .and_then(|entries| {
            entries.iter().find_map(|entry| {
                ...
                let entry_token = pair.first().and_then(Value::as_u64)?;
                if entry_token != token_id {
                    return None;
                }
                value_as_f64(&pair[1])
            })
    ...
    Some(AccountSummary {
        ...
        account_value: total,
        margin_used: hold,
        available_margin: available_after_maintenance,
        is_locked: available_after_maintenance <= 0.0,
    })
}
...
async fn post_json_with_client(client: &Client, url: &str, payload: &Value) -> Result<Value> {
    let response = client
        .post(url)
        .json(payload)
        .send()
...
async fn post_private_first(
    ...
    if let Some(private_url) = private_url {
        match post_json_with_client(client, private_url, payload).await {
            Ok(data) => return Ok(data),
            Err(err) => debug!("private node request failed, falling back to public: {err}"),
        }
    }
    post_json_with_client(client, public_url, payload).await
}
...
pub(crate) fn normalize_hl_symbol(symbol: &str) -> String {
...
fn safe_proxy_label(proxy: &str) -> String {
...
fn should_try_proxy_fallback(err: &anyhow::Error) -> bool {
    let text = err.to_string();
    is_rate_limited_error(&text)
